...
ai_idf = tfidf.fit_transform(ai.text)

# compress the dimensions with UMAP
from umap import UMAP
umap = UMAP()
u2 = umap.fit_transform(ai_idf)

# lets plot this up
u2df = pd.DataFrame(u2, columns=["e1", "e2"])
sns.scatterplot(data=u2df, x="e1", y="e2", hue=ai.intent, alpha=.5) 
plt.show()
...
PROJECT = 'ba820-avs'
SQL = "SELECT * from `questrom.datasets.bruins_twitter` WHERE hour IN (0, 1, 2, 3) ORDER BY created_at asc"
bt = pd.read_gbq(SQL, PROJECT)

bt.head()
bt.shape

# define a function that extracts the score per doc
def sentiment(text):
    return afinn.score(text)

# pandas --apply
bt['sentiment'] = bt.text.apply(sentiment)

bt.head()

# test the hypothesis of changing sentiment over time
bt['created'] = pd.to_datetime(bt.created_at)

bt.info()

# sort the records
bt.sort_values('created', inplace=True)
bt.reset_index(inplace=True)
bt.head(3)

sns.lineplot(x=bt.created, y=bt.sentiment)
plt.show()

# aggregation by hour
bt.groupby('hour').agg({'sentiment':['size','mean']})
...
# spacy naturally wants to operate on a document
...
# sent = []
# for doc in corpus:
#   sent.append(SpacyTextBlob(doc).sentiment.polarity)
